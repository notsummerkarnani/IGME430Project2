...
IngredientSchema.statics.findByName = (ownerId, ingredientName, callback) => {
    const search = {
        owner: mongoose.Types.ObjectId(ownerId),
        name: ingredientName,
    };
    return IngredientModel.findOne(search).exec(callback);
};
...
IngredientSchema.statics.findByID = (ingredientID, callback) => {
    const search = {
        _id: mongoose.Types.ObjectId(ingredientID),
    };
    return IngredientModel.find(search).lean().exec(callback);
};
